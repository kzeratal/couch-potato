...
export async function work(argv: string[]): Promise<void> {
  ...
  const systemPrompt = buildSystemPrompt(shadow, cfg);
...
function buildSystemPrompt(shadow: string, cfg: ShadowConfig): string {
...
Each directory in the shadow has a \`_MAP.md\` with: \`purpose\` / \`entries\` / \`deps\` / \`gotchas\`.

## Rules

1. Map first. Before grep/glob on source for orientation, Read \`<shadow>/<dir>/_MAP.md\`. Start with \`<shadow>/_MAP.md\` for top-level orientation.
2. Shadow is read-only. Edit real source in CWD (this session's working directory).
3. Trust source over map. Maps are point-in-time snapshots; if real source disagrees, the map is stale until next \`couch-potato sync\`.
4. Brief subagents. Subagents do not inherit this prompt. When delegating, hand them the shadow path \`${shadow}\` and tell them to Read \`_MAP.md\` before grep.
5. Auto-sync. After the task ends, the wrapper runs \`couch-potato sync\` automatically.
`;
}
